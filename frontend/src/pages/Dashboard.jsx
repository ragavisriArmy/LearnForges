...
export default function Dashboard() {
    ...
    const [sbBtnPos, setSbBtnPos] = useState("75%");
        ...
        <div style={{ maxWidth: '1240px', margin: '2rem auto', padding: '0 1.5rem', color: '#f8fafc', fontFamily: 'system-ui, sans-serif' }}>
            ...
            <header className="responsive-header">
                ...
                <nav className="responsive-nav">
                    ...
                    <button onClick={() => { setActiveTab('profile'); setSelectedCourseId(null); }} style={{ padding: '0.5rem 1rem', borderRadius: '6px', border: 'none', cursor: 'pointer', fontWeight: '600', backgroundColor: activeTab === 'profile' ? '#10b981' : 'transparent', color: '#fff' }}>My Analytics</button>
                </nav>
            </header>
            ...
            <div className="responsive-grid">
                <main style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
                    ...
                    {selectedCourseId !== null ? (
                        <div style={{ backgroundColor: '#1e293b', padding: '2rem', borderRadius: '12px', border: '1px solid #334155' }}>
                            {!quizFinished ? (
                                <div>
                                    ...
                                    <button onClick={submitQuiz} style={{ width: '100%', padding: '1rem', backgroundColor: '#10b981', color: '#fff', border: 'none', borderRadius: '8px', cursor: 'pointer', fontWeight: '700' }}>Submit Attempt</button>
                                </div>
                            ) : (
                        ...
                        <div>
                                ...
                                <div style={{ backgroundColor: '#1e293b', padding: '2rem', borderRadius: '12px', border: '1px solid #334155' }}>
                                    ...
                                    <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
                                        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                                            ...
                                            <label><span style={{ fontSize: '0.85rem', color: '#94a3b8' }}>CANVAS FILL BACKGROUND</span>
                                                <select value={sbBg} onChange={(e) => setSbBg(e.target.value)} style={{ width: '100%', padding: '0.6rem', borderRadius: '6px', backgroundColor: '#0f172a', border: '1px solid #334155', color: '#fff', marginTop: '0.4rem' }}>
                                                    <option value="linear-gradient(135deg, #1e3a8a, #0f172a)">Midnight Deep Blue</option>
                                                    <option value="linear-gradient(135deg, #115e59, #0f172a)">Emerald Matrix Green</option>
                                                </select>
                                            </label>
                                        </div>
                                        <div style={{ display: 'flex', justifyContent: 'center' }}>
                                            <div style={{ width: '180px', height: '320px', borderRadius: '20px', background: sbBg, border: '4px solid #475569', position: 'relative', overflow: 'hidden' }}>
                                                ...
                                                <button style={{ position: 'absolute', top: sbBtnPos, left: '10%', width: '80%', padding: '0.5rem', backgroundColor: '#3b82f6', color: '#fff', border: 'none', borderRadius: '20px', fontWeight: '700', fontSize: '0.8rem' }}>Get Started</button>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            ...
                            {activeTab === 'admin' && (
                                <div style={{ backgroundColor: '#1e293b', padding: '2rem', borderRadius: '12px', border: '1px solid #334155' }}>
                                    <h2 style={{ color: '#fbbf24', margin: '0 0 1.5rem 0' }}>Custom Track Injection Desk</h2>
                                    <form onSubmit={handleAdminSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                                        ...
                                        <input type="text" placeholder="Exact Correct Value Matching Key" required value={adminForm.correct} onChange={e => setAdminForm({...adminForm, correct: e.target.value})} style={{ width: '100%', padding: '0.6rem', borderRadius: '4px', backgroundColor: '#0f172a', border: '1px solid #334155', color: '#fff' }}/>
                                        <button type="submit" style={{ padding: '0.75rem', backgroundColor: '#fbbf24', color: '#1e293b', border: 'none', borderRadius: '6px', cursor: 'pointer', fontWeight: '700' }}>Commit Custom Matrix Array</button>
                                    </form>
                                </div>
                            )}
                ...
                <aside style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                    <div style={{ backgroundColor: '#1e293b', padding: '1.5rem', borderRadius: '12px', border: '1px solid #334155' }}>
                        <h3 style={{ margin: '0 0 1rem 0', fontSize: '1.1rem' }}>Global Skill Analytics</h3>
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.85rem' }}><span style={{ color: '#94a3b8' }}>Evaluations:</span><strong>{stats.totalAttempts}</strong></div>
                            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.85rem' }}><span style={{ color: '#94a3b8' }}>Avg Accuracy:</span><strong style={{ color: '#3b82f6' }}>{stats.averageAccuracy}%</strong></div>
                        </div>
                    </div>
                </aside>
            </div>
